# clear blocks by matching 3 or more
...
class Stack():
   ...
   def fill(self):
      for b in range(self.size):
         self.blocks.append(Block(self.board,self,(self.pos,b),random.randint(0,NUMCOLORS-1)))
#         print self.blocks[b],b, 'color', self.blocks[b].color
   ...
   def printStack(self):
      ...
      #for b in range(self.size):
         if dont: break
         if self.blocks[b].breaking:
            print '!',
         else:
            print self.blocks[b].color, # terminal
         self.blocks[b].render()
         self.checked = False
   def match(self,pos):
      count = 0
      matches = []
...
def inGame():
   ...
   for event in pygame.event.get():
      ...
      if event.type == KEYDOWN:
         ...
         if event.key == K_j:
            if player.size == 0:
               if player.popSize < player.board.stacks[pos].size:
                  if not player.board.stacks[pos].blocks[player.board.stacks[pos].size-1-player.popSize].breaking: # ew.
                     player.popSize += 1
            else:
               player.drop(pos)
